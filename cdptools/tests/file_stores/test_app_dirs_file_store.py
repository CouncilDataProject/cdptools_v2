...
def mock_compute_sha256():
    with mock.patch(
        "cdptools.file_stores.file_store.FileStore.compute_sha256_for_file"
    ) as MockSHA:
        MockSHA.return_value = (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "does_not_exist.mp4",
            "not_going_to_exist.mp4",
            marks=pytest.mark.raises(exception=FileNotFoundError),
        ),
    ],
)
def test_download_file(
    fs, mock_compute_sha256, tmpdir, example_video, filename, save_path
):
    # Upload file
    tmp_input_path = shutil.copyfile(example_video, tmpdir / "tmp.mp4")
