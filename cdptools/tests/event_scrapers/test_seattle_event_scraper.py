...
@pytest.fixture
def single_event_soup(data_dir):
    with open(data_dir / "example_seattle_channel_single_event.html", "r") as read_html:
        return BeautifulSoup(read_html.read(), features="html.parser")
...
def test_parse_single_seattle_channel_event(single_event_soup):
    event = SeattleEventScraper._parse_single_seattle_channel_event_by_main_content(
